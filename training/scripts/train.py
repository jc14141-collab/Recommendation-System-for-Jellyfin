#!/usr/bin/env python3
"""
IndieFlicks Recommender Training Script (K8s / S3 Version)
==========================================================
Fine-tunes MLP on pre-computed user/movie embeddings streamed from MinIO (S3).
Supports loading pretrained weights and limiting batches per epoch.
Saves trained model back to MinIO for persistence.
...
def upload_model_to_s3(s3_cfg, local_path, bucket, key):
    """Upload trained model to MinIO for persistence."""
...
def main():
    parser = argparse.ArgumentParser(description="IndieFlicks MLP Fine-tuning (K8s)")
    ...
    if pretrained_cfg and pretrained_cfg.get("enabled", False):
        local_weight_path = "/app/pretrained_weights.pt"
    ...
    # MLflow run
    with mlflow.start_run(run_name="mlp-finetune"):
        mlflow.log_param("model_type", "mlp")
        mlflow.log_param("device", str(device))
        mlflow.log_param("platform", platform.node())
        mlflow.log_param("param_count", param_count)
        mlflow.log_param("data_version", data_cfg.get("version", "auto"))
        ...
        if model_persist:
            upload_model_to_s3(
                s3_cfg, model_save_path,
                model_persist["s3_bucket"],
                model_persist["s3_key"],
            )
